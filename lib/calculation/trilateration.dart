...
class Trilateration {
  ...
  List<int> trilaterationMethod(List<Coordinate> coordinateList) {
    const double maxDistance = 10000.0;  // 최대 거리 상수
    var matrixA = [];
    var matrixB = [];
    ...
    for (int idx = 1; idx <= coordinateList.length - 1; idx++) {
      var definedRadius = coordinateList[idx].radius > maxDistance
          ? maxDistance
    ...
    var matrixInverse = invert2x2Matrix(m2d.dot(matrixATranspose, matrixA)); // 역행렬 계산
  ...
  // 2x2 행렬의 역행렬을 계산하는 메서드
  List invert2x2Matrix(List list) {
    var shape = list.shape;
    var temp = List.filled(shape[1], 0.0)
        .map((e) => List.filled(shape[0], 0.0))
        .toList();
